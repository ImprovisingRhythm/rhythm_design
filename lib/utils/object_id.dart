...
class ObjectId implements Comparable<ObjectId> {
  ...
  factory ObjectId.fromHexString(String hexString) {
    final b = _parseHexString(hexString);
  ...
  factory ObjectId.fromBytes(List<int> bytes) {
    ...
    final timestamp = _makeInt(bytes[0], bytes[1], bytes[2], bytes[3]);
    final machineId = _makeInt(0, bytes[4], bytes[5], bytes[6]);
    final processId = _makeInt(0, 0, bytes[7], bytes[8]);
    final counter = _makeInt(0, bytes[9], bytes[10], bytes[11]);
  ...
  String toHexString() {
    final charCodes = List<String?>.filled(24, null);
  ...
  int compareTo(ObjectId other) {
    final byteList = toBytes();
    final otherByteList = other.toBytes();
