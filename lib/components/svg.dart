...
class SvgProvider extends ImageProvider<SvgImageKey> {
  ...
  Future<SvgImageKey> obtainKey(ImageConfiguration configuration) {
    ...
    final color = this.color ?? const Color(0x00000000);
  ...
  @override
  ImageStreamCompleter load(SvgImageKey key, DecoderCallback decode) {
  ...
  static Future<ImageInfo> _loadAsync(SvgImageKey key) async {
    final rawSvg = await rootBundle.loadString(key.assetName);
    final svgRoot = await svg.fromSvgString(rawSvg, key.assetName);
