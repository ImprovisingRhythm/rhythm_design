...
/// the same values passed into the [SliverRefreshControl].
///
...
/// A callback function that's invoked when the [SliverRefreshControl] is
/// pulled a `refreshTriggerPullDistance`. Must return a [Future]. Upon
/// completion of the [Future], the [SliverRefreshControl] enters the
...
/// the [SliverNavigationBar], this widget will:
///
...
///    the scrollable.
///  * By using [App], which always uses a [ScrollConfiguration]
...
/// sliver such as [SliverNavigationBar] and your main scrollable
/// content's sliver.
///
/// See also:
///
...
///    [SliverRefreshControl] is part of the scrollable and actively occupies
///    scrollable space.
class SliverRefreshControl extends StatefulWidget {
  ...
  /// Retrieve the current state of the SliverRefreshControl. The same as the
  /// state that gets passed into the [builder] function. Used for testing.
  @visibleForTesting
  static RefreshIndicatorMode state(BuildContext context) {
    final state = context.findAncestorStateOfType<_SliverRefreshControlState>();
  ...
  static Widget _buildIndicatorForRefreshState(
    ...
    switch (refreshState) {
      ...
      case RefreshIndicatorMode.done:
        return ActivityIndicator(radius: radius);
