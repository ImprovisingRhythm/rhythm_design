...
// Minimum horizontal padding between [TimerPicker]
//
...
TextStyle _themeTextStyle(BuildContext context, {bool isValid = true}) {
  ...
  final style = theme.pickerTextStyle;

  return isValid ? style : style.copyWith(color: theme.unselectedTextColor);
}
...
const Widget _leftSelectionOverlay =
    PickerDefaultSelectionOverlay(capRightEdge: false);

const Widget _centerSelectionOverlay =
    PickerDefaultSelectionOverlay(capLeftEdge: false, capRightEdge: false);

const Widget _rightSelectionOverlay =
    PickerDefaultSelectionOverlay(capLeftEdge: false);
          ...
          FlutterError.reportError(
            FlutterErrorDetails(
              exception: FlutterError(
                ...
                'DatePicker because the parent is too narrow at '
                '${size.width}px.\n'
...
/// Different display modes of [DatePicker].
///
/// See also:
///
///  * [DatePicker], the class that implements different display modes
///    of the iOS-style date picker.
///  * [Picker], the class that implements a content agnostic spinner UI.
enum DatePickerMode {
  /// Mode that shows the date in hour, minute, and (optional) an AM/PM designation.
  /// The AM/PM designation is shown only if [DatePicker] does not use 24h format.
...
// Different types of column in DatePicker.
...
///
/// There are several modes of the date picker listed in [DatePickerMode].
///
...
/// See also:
///
///  * [TimerPicker], the class that implements the iOS-style timer picker.
///  * [Picker], the class that implements a content agnostic spinner UI.
class DatePicker extends StatefulWidget {
  /// Constructs an iOS style date picker.
  ///
  /// [mode] is one of the mode listed in [DatePickerMode] and defaults
  /// to [DatePickerMode.dateAndTime].
  ///
  /// [onDateTimeChanged] is the callback called when the selected date or time
  /// changes and must not be null. When in [DatePickerMode.time] mode,
  /// the year, month and day will be the same as [initialDateTime]. When in
  /// [DatePickerMode.date] mode, this callback will always report the
  ...
  /// In [DatePickerMode.time] mode, [minimumDate] should typically be
  ...
  /// In [DatePickerMode.time] mode, [maximumDate] should typically be
  /// on the same date as [initialDateTime], as the picker will not limit the
  ...
  /// [DatePickerMode.date] mode. Defaults to 1 and must not be null.
  ///
  /// [maximumYear] is the maximum year that the picker can be scrolled to in
  /// [DatePickerMode.date] mode. Null if there's no limit.
  ///
  ...
  ///
  /// [dateOrder] determines the order of the columns inside [DatePicker] in date mode.
  /// Defaults to the locale's default date format/order.
  DatePicker({
    Key? key,
    this.mode = DatePickerMode.dateAndTime,
  ...
  /// The mode of the date picker as one of [DatePickerMode].
  /// Defaults to [DatePickerMode.dateAndTime]. Cannot be null and
  ...
  ///
  /// In [DatePickerMode.time] mode, a time becomes unselectable if the
  ...
  State<StatefulWidget> createState() {
    ...
    switch (mode) {
      ...
      case DatePickerMode.dateAndTime:
        return _DatePickerDateTimeState();
      case DatePickerMode.date:
        return _DatePickerDateState(dateOrder: dateOrder);
    }
  }
...
class _DatePickerDateTimeState extends State<DatePicker> {
  ...
  double _getEstimatedColumnWidth(_PickerColumnType columnType) {
    if (estimatedColumnWidths[columnType.index] == null) {
      estimatedColumnWidths[columnType.index] =
          DatePicker._getColumnWidth(columnType, localizations, context);
    }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.mode == DatePickerMode.dateAndTime) {
      if (localizations.datePickerDateTimeOrder ==
              DatePickerDateTimeOrder.timeDayPeriodDate ||
          localizations.datePickerDateTimeOrder ==
...
class _DatePickerDateState extends State<DatePicker> {
  _DatePickerDateState({
...
/// Different modes of [TimerPicker].
///
/// See also:
///
///  * [TimerPicker], the class that implements the iOS-style timer picker.
...
///
/// There are several modes of the timer picker listed in [TimerPickerMode].
///
/// The picker has a fixed size of 320 x 216, in logical pixels, with the exception
/// of [TimerPickerMode.hms], which is 330 x 216. If the parent widget
...
class TimerPicker extends StatefulWidget {
  ...
  ///
  /// [mode] is one of the modes listed in [TimerPickerMode] and
  /// defaults to [TimerPickerMode.hms].
  ///
...
class _TimerPickerState extends State<TimerPicker> {
  ...
  void initState() {
    ...
    if (widget.mode != TimerPickerMode.ms) {
      selectedHour = widget.initialTimerDuration.inHours;
    }

    if (widget.mode != TimerPickerMode.hm) {
  ...
  @override
  void didUpdateWidget(TimerPicker oldWidget) {
    super.didUpdateWidget(oldWidget);

    assert(
      oldWidget.mode == widget.mode,
      "The TimerPicker's mode cannot change once it's built",
    );
  }
  ...
  Widget _buildMinutePicker(
    ...
    return Picker(
      ...
      offAxisFraction: _calculateOffAxisFraction(
        ...
        widget.mode == TimerPickerMode.ms ? 0 : 1,
      ),
      itemExtent: _kItemExtent,
      squeeze: _kSqueeze,
      looping: true,
  ...
  Widget _buildSecondPicker(
    ...
    return Picker(
      ...
      offAxisFraction: _calculateOffAxisFraction(
        additionalPadding.start,
        widget.mode == TimerPickerMode.ms ? 1 : 2,
      ),
      itemExtent: _kItemExtent,
      squeeze: _kSqueeze,
      looping: true,
  ...
  TextStyle _textStyleFrom(BuildContext context, [double magnification = 1.0]) {
    final textStyle = ThemeProvider.of(context).pickerTextStyle;
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        if (widget.mode == TimerPickerMode.hms) {
        ...
        if (constraints.maxWidth < totalWidth) {
          ...
          pickerColumnWidth =
              totalWidth / (widget.mode == TimerPickerMode.hms ? 3 : 2);
        }
